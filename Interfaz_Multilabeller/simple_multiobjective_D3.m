function y = simple_multiobjective_D3(x)
...
% return argument y.
global numcOp functiontoeval valuestemp mu covs numberOfVariables nevalD nclas
...
for j=1:nevalD
  ...
  %  covs(:,:)=covso(j,:,:);
    y = y+x(j+nclas)*mvnpdf([(x(1:nclas))],mu{numcOp,j},covs{numcOp,j});
end
% x(1:nclas)=fliplr(x(1:nclas))
% for j=1:size(valuestemp,2)
%  yt(1)=(valuestemp{j}(1));
